...
export default function EmployeeDashboard() {
  ...
  const goalsApproved = goals.filter(goal => goal.status === 'approved').length;
      ...
      <main className="content">
        <div className="stats-row">
          <div className="stat">
            ...
            <div className="stat-val">{goalsApproved}</div>
            <div className="stat-label">Goals approved</div>
            <div className="stat-delta" style={{ background: 'var(--green-bg)', color: 'var(--green)' }}>{goals.length ? `${goals.length} total` : 'No goals yet'}</div>
          </div>
          ...
          <div className="stat">
            ...
            <div className="stat-val">{totalWeightage}%</div>
            <div className="stat-label">Weightage used</div>
            <div className="stat-delta" style={{ background: weightageBalanced ? 'var(--green-bg)' : 'var(--amber-bg)', color: weightageBalanced ? 'var(--green)' : 'var(--amber)' }}>
              {weightageBalanced ? 'Balanced ✓' : 'Needs review'}
            </div>
          </div>
          ...
          <div className="stat">
            ...
            <div className="stat-val">{goals.length ? Math.round(goals.reduce((sum, goal) => sum + Number(goal.achievements?.[0]?.computed_score || 0), 0) / goals.length) : 0}%</div>
            <div className="stat-label">Q1 avg progress</div>
            <div className="stat-delta" style={{ background: 'var(--amber-bg)', color: 'var(--amber)' }}>{goals.length ? 'Live from goal data' : 'No progress yet'}</div>
          </div>
          ...
          <div className="stat">
            ...
            <div className="stat-val">{weightageBalanced ? 'Balanced' : 'Review'}</div>
            <div className="stat-label">Next check-in</div>
            <div className="stat-delta" style={{ background: 'var(--surface2)', color: 'var(--text3)' }}>{goals.length ? 'Q2 window' : 'Awaiting goals'}</div>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div className="card">
              <div className="card-header">
                ...
                <span className={`badge ${weightageBalanced ? 'badge-green' : 'badge-amber'}`}>{totalWeightage}% {weightageBalanced ? 'balanced' : 'needs review'}</span>
              </div>
                ...
                <div className="w-viz">
                  {goals.map((goal, i) => (
                    <div 
                      ...
                      className="w-seg" 
                      style={{ 
                        width: `${Math.max(0, Math.min(100, Number(goal.weightage || 0)))}%`, 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginTop: '12px' }}>
                  {goals.map((goal) => (
                    <div key={goal.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11.5px' }}>
                      ...
                      <span style={{ color: 'var(--text3)', fontFamily: "'Geist Mono', monospace" }}>{Number(goal.weightage || 0)}%</span>
                    </div>
                  ))}
                </div>
              </div>
